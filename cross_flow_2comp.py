import pandas as pd
import cross_flow_2comp as cf
import numpy as np
from vincent.colors import brews


def cross_flow():
    parameters = list(pd.read_csv('input_coker_2comp.txt', header=None)[0])
    columns = ('CO2','H2O')

    A = parameters[0]
    dA = 500
    N = int(A / dA)
    y0 = [0, 1]#[parameters[i] for i in range(14, 20)]
    x0 = [1, 0] #[parameters[i] for i in range(5, 11)]
    M = len(x0)
    mi = [0] * M
    #pV = [0] * (N+1)
    pV = parameters[11]
...
def permeabilityCalc(N, k, pM, x, y, pV, M, Q, parameters):
    ...
    Q_gpu[1] = 1000 #0.0014 * np.exp(.0637*aw) / s
    #Q_gpu[2] = 10#10
    #Q_gpu[3] = 10#10
    #Q_gpu[4] = 1000
    #Q_gpu[5] = 1000
